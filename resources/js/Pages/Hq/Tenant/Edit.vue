...
const props = defineProps({
    show: Boolean,
    title: String,
    tenant: Object // ✨ A meglévő tenant adatok
});
...
const rules = computed(() => ({
    ...
    database: { required },
    username: { required },
    password: { required }, // vagy opcionális: { required: false }
}));
...
const update = async () => {
    ...
    try {
        await TenantService.updateTenant(form.value.id, { ...form.value });

        emit('updated', form.value);

        toast.add({
            severity: 'success',
            summary: 'Módosítás sikeres',
        ...
        if (errors) {
            ...
            console.error('Módosítás sikertelen', e);
        }
    } finally {
